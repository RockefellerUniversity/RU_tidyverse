...
# chrs <- select(TxDb.Hsapiens.UCSC.hg38.knownGene, keys = keys, columns = "TXCHROM", keytype = "GENEID")
# 
# # We can use the ENTREZ ID to get a list of genes with grange of their exons
# geneExons <- exonsBy(TxDb.Hsapiens.UCSC.hg38.knownGene,by = "gene")[keys]
# 
...
# We can use the ENTREZ ID to look up the chromosome the gene resides on
chrs <- AnnotationDbi::select(TxDb.Hsapiens.UCSC.hg38.knownGene, keys = keys, columns = "TXCHROM", keytype = "GENEID")
...
## -----------------------------------------------------------------------------
#There's a wide range of writing  options. Can specify the delmiter directly or use a specific function
write_delim(tidy_counts_expressed, 'dataset/expressed_genes_output.csv', delim =',')

write_csv(tidy_counts_expressed, 'dataset/expressed_genes_output.csv')
...
## -----------------------------------------------------------------------------
tidy_counts_expressed_norm <- read_csv("dataset/expressed_genes_output_norm.csv")
...
brc <- c("Tom", "Matt", "Doug", "Wei", "Jenelys")
...
## -----------------------------------------------------------------------------
brc2 <- c("Tom  ", "M att", "  Doug", "Wei", "Jen   elys")
